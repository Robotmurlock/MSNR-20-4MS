...
``Debager kao debger. Kao da tu ima nekih bitnih razlika.`` bi pomislio neko ko nije bio u prilici da debaguje teške bagove bazičnim debagerom. Nakon čitanja rada čitalac će biti upoznat sa mogućnostima LLDB debagera na različitima platformama i da li je prava odluka ostati na debageru koju koristi ili se prebaciti na LLDB.
...
\item \textbf{Neformalno debagovanje}: Neformalno debagovanje predstavlja jednostavan i površan pristup i čine ga dva koraka.
  ...
  \end{enumerate}
  Ovaj metod se ne preporučuje u praksi i često može da proizvede nove probleme, pogotovo ako vršimo mnogo sitnih popravki za koje nismo sigurni da će rešiti problem. Ponekad, ako su u pitanju sitne greške, ovaj metod se može oprezno koristiti jer dovodi do brzog rešenja problema \cite{bagovi_smalkov}.
...
\item \textbf{Empirijski naučni metod}:
  ...
  \end{enumerate}
  \indent Uopšteno gledano, ovo je najbolji pristup debagovanju. Često je jako zahtevan i oduzima dosta vremena, ali je sa druge strane temeljan i koncizan \cite{bagovi_smalkov}.
